<x-admin-layout>
    ...
    <div class="space-y-6">
        ...
        <div class="bg-blue-50 border border-blue-200 rounded-lg px-4 py-3 text-sm text-blue-800">
            <strong>Roster import:</strong> Export each tab from your Google Sheet as CSV (File → Download → CSV).
            Upload one file per program below. Required column: <code class="bg-blue-100 px-1 rounded">Name</code>.
            Optional: <code class="bg-blue-100 px-1 rounded">Role</code> (put "Coach" on the coach's row),
            <code class="bg-blue-100 px-1 rounded">Jersey</code>.
        </div>
        ...
        <div class="bg-white rounded-lg shadow">
            <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                ...
                @else
                    <span class="text-sm text-amber-500 italic">No coach yet</span>
                @endif
            </div>
            ...
            <div class="px-6 py-5 space-y-6">
                ...
                {{-- CSV Upload --}}
                <div>
                    <h4 class="text-sm font-medium text-gray-700 mb-2">Import Roster from CSV</h4>
                    <form method="POST" action="{{ route('admin.teams.import', $team) }}" enctype="multipart/form-data"
                        ...
                        <button type="submit"
                                class="px-4 py-1.5 bg-blue-600 text-white text-sm rounded hover:bg-blue-700 whitespace-nowrap">
                            Upload &amp; Import
                        </button>
                    </form>
                    @if($team->players->count())
                        <p class="mt-1 text-xs text-gray-400">Uploading a new file replaces all existing players.</p>
                    @endif
                </div>
                ...
                <form method="POST" action="{{ route('admin.teams.update', $team) }}">
                    @csrf @method('PUT')
                    <h4 class="text-sm font-medium text-gray-700 mb-3">Program Details</h4>
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 text-sm">
                        ...
                        <div>
                            ...
                            <input type="text" name="coach_name" value="{{ old('coach_name', $team->coach_name) }}"
                                   placeholder="Pulled from CSV or enter manually"
                                   class="w-full border border-gray-300 rounded px-3 py-1.5 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none">
                        </div>
